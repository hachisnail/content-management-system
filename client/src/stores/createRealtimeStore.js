...
export const createRealtimeStore = (resourceName) => {
  return create((set, get) => ({
    ...
    fetchQuery: async (queryString, { silent = false } = {}) => {
      ...
      if (!silent) {
        set((state) => ({ queries: new Map(state.queries).set(queryString, { ...existing, loading: true }) }));
      }
      try {
        const queryParams = JSON.parse(queryString);
        const response = await api.get(`/${resourceName}`, { params: queryParams });
        const data = response?.data || response?.rows || response || [];
        const meta = response?.meta || (response?.count !== undefined ? { totalItems: response.count } : { totalItems: data.length });
        
        set((state) => ({ queries: new Map(state.queries).set(queryString, { data, meta, loading: false, error: null }) }));
      } catch (error) {
        set((state) => ({ queries: new Map(state.queries).set(queryString, { ...existing, loading: false, error: error.message }) }));
      }
    },
    ...
    fetchEntity: async (id, { silent = false } = {}) => {
      ...
      if (!silent) {
        set((state) => ({ entities: new Map(state.entities).set(idStr, { ...existing, loading: true }) }));
      }
      try {
        const data = await api.get(`/${resourceName}/${idStr}`);
        set((state) => ({ entities: new Map(state.entities).set(idStr, { data, loading: false, error: null }) }));
      } catch (error) {
        set((state) => ({ entities: new Map(state.entities).set(idStr, { ...existing, loading: false, error: error.message }) }));
      }
    },
    ...
    refetchAllData: () => {
      ...
      get().queries.forEach((_value, key) => get().fetchQuery(key, { silent: true }));
      get().entities.forEach((_value, key) => get().fetchEntity(key, { silent: true }));
    },
    
    // --- SOCKET HANDLERS ---

    handleCreated: (newItem) => {
      console.log(`%c[Store: ${resourceName}] Created`, 'color: #22C55E;', newItem);
      get().queries.forEach((_value, key) => get().fetchQuery(key, { silent: true }));
    },

    handleUpdated: (updatedItem) => {
      const idStr = String(updatedItem.id);
      console.log(`%c[Store: ${resourceName}] Updated ID: ${idStr}`, 'color: #3B82F6;', updatedItem);
      
      if (get().entities.has(idStr)) {
        set((state) => {
          const existing = state.entities.get(idStr) || {};
          const updatedEntity = { ...existing.data, ...updatedItem };
          return { entities: new Map(state.entities).set(idStr, { ...existing, data: updatedEntity }) };
        });
      }
      get().queries.forEach((_value, key) => get().fetchQuery(key, { silent: true }));
    },
    ...
    handleDeleted: (deletedItem) => {
      ...
      console.log(`%c[Store: ${resourceName}] Deleted ID: ${idStr}`, 'color: #EF4444;', deletedItem);
    ...
    handleConnect: () => {
       console.log(`[Store: ${resourceName}] Socket Connected. Stopping Fallback Polling.`);
    ...
    handleDisconnect: () => {
        console.warn(`[Store: ${resourceName}] Socket Disconnected. Starting Fallback Polling...`);
    ...
    subscribe: () => {
      ...
      if (currentCount === 0 && !get().isSubscribed) {
          ...
          const { 
            ...
            handleConnect, handleDisconnect, startPolling 
          } = get();
          ...
          if (socket.connected) {
             ...
             console.log(`[Store: ${resourceName}] Started offline. Enabling polling.`);
             startPolling();
    ...
    unsubscribe: () => {
      ...
      if (currentCount - 1 === 0) {
          ...
          const { 
            handleCreated, handleUpdated, handleDeleted, 
            handleConnect, handleDisconnect, stopPolling 
          } = get();
