...
// [FIX] Export the socketService object so other services can import it
...
export const initSocket = (httpServer, corsOptions) => {
  ...
  io.on('connection', async (socket) => {
    ...
    socket.on('disconnect', async () => {
      // Check if this user has any OTHER active sockets open (e.g., other tabs)
...
// --- Scalable Event Listeners ---
  ...
  // [FIX] Broadcast DB Changes with Debug Logging
  [EVENTS.DB_CREATE, EVENTS.DB_UPDATE, EVENTS.DB_DELETE].forEach(evtType => {
    appEvents.on(evtType, (payload) => {
      if (payload && payload.resource) {
        ...
        // [DEBUG] Log the broadcast to verify functionality
        console.log(`[SOCKET] Broadcasting ${evtType} to room: ${room} | Data: ${payload.data}`);
