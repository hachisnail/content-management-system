...
import { LiveStatusBadge } from '../components/user-manager/LiveStatusBadge';
import { ArrowLeft, Save, UserX, Trash2, Power, Phone, Calendar, Mail } from 'lucide-react';
import { userApi } from '../../../api/users.api';
import { useAuth } from '../../auth/hooks/useAuth';
import { ROLE_HIERARCHY, ROLES } from '../../../config/roles';
import { usePermission } from '../../../providers/PermissionProvider'; // [NEW]
...
export const UserDetailsPage = () => {
  ...
  const { can, canManageUser } = usePermission();
  ...
  const handleAction = async (type) => {
    const actionMap = {
      ...
      disable: { title: "Disable User?", msg: "User will be blocked from logging in.", fn: () => userApi.disable(id, !user.isActive), variant: 'warning' },
      enable: { title: "Enable User?", msg: "Restore access for this user?", fn: () => userApi.disable(id, !user.isActive), variant: 'info' },
      delete: { title: "Soft Delete?", msg: "Move user to Recycle Bin?", fn: () => userApi.delete(id), variant: 'danger' }
    };
  ...
  if (loading || !user) return <div className="p-10 text-center"><span className="loading loading-spinner loading-lg"></span></div>;
    ...
    <PageContainer 
      ...
      actions={
        <div className="flex gap-2">
          ...
          {!isEditing ? (
            isManageable && can('updateAny', 'users').granted && (
          ...
          <div className="card bg-base-100 border border-base-200 shadow-sm h-full">
            <div className="card-body">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="form-control md:col-span-2 mt-4"><label className="label"><span className="label-text">Member Since</span></label><input type="text" className="input input-bordered" disabled value={new Date(user.createdAt).toLocaleString()} /></div>
              </div>
            </div>
          </div>
        </div>
