...
import { ArrowLeft, User, Clock, Shield, Activity, Globe, Monitor } from 'lucide-react';
// [FIX] Import the object 'auditApi', not a non-existent function 'getAuditLog'
...
export const AuditLogDetailsPage = () => {
  ...
  useEffect(() => {
    const fetchLog = async () => {
      try {
        // [FIX] Call the method on the object
          ...
          <div className="card bg-base-100 border border-base-200 shadow-sm">
            <div className="card-body p-6">
              <div className="flex flex-wrap items-center gap-4">
                ...
                <span className="font-mono font-bold text-lg">{log.resource}</span>
                {log.resourceId && (
                   <span className="badge badge-ghost font-mono">{log.resourceId}</span>
                )}
              </div>
            </div>
          </div>
