...
import { useUserManager } from '../../system/hooks/useUserManager';
...
export const UserDirectoryPage = () => {
  ...
  // Columns ... (Keep existing implementation)
  const activeColumns = [
    {
      key: 'name',
      label: 'User',
    ...
    },
    {
      ...
      label: 'Activity',
      sortable: true, 
      render: (user) => <LiveStatusBadge user={user} variant="text" />
    },
    {
      key: 'roles',
      label: 'Role',
  ...
  const pendingColumns = [
    ...
    },
    {
      key: 'createdAt',
      label: 'Sent Date',
      render: (user) => <span className="text-xs opacity-60">{new Date(user.createdAt).toLocaleDateString()}</span>
    }
  ];
  ...
  const activeActions = (user) => {
    ...
    return (
      <>
        <button onClick={() => navigate(`/system/users/${user.id}`)} className="w-full text-left text-sm py-2 px-3 hover:bg-base-200 flex items-center gap-2">
          ...
          <>
            ...
            {user.isOnline && (
              <button 
                onClick={() => openConfirm(
                  "Force Logout?",
                  "This will immediately terminate the user's active session. They will be redirected to the login page.",
            ...
            {can('deleteAny', 'users').granted && (
              <button 
                onClick={() => openConfirm(
                  "Move to Recycle Bin?",
                  "This user will be soft-deleted. You can restore them later from the Recycle Bin.",
  ...
  const pendingActions = (user) => (
    <>
      <button 
        onClick={() => openConfirm(
          "Resend Invitation?",
          `Send a new invitation email to ${user.email}? This will extend the expiration date.`,
          () => actions.resendInvite(user.id),
          "info"
        )}
  ...
  return (
    <PageContainer
      ...
      actions={
        can('createAny', 'users').granted && (
          <button onClick={() => navigate('/system/users/invite')} className="btn btn-primary btn-sm gap-2">
