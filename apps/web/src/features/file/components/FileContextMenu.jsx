...
import { Eye, Info, Edit2, Globe, Lock, Trash2, RefreshCw, Shield, FolderOpen } from "lucide-react";

// [FIX] Accept 'canModify' prop
export const FileContextMenu = ({ contextMenu, onClose, onAction, section, canModify = true }) => {
  ...
  const isFolder = 
    ...
    (section === 'recycle' && item.resourceType !== 'files');
  ...
  return (
    ...
    >
      {section === 'library' ? (
        <>
          {isFolder && (
            <button onClick={() => onAction('open', item)} className="w-full text-left px-4 py-2 hover:bg-base-200 flex items-center gap-2 text-sm font-medium">
          ...
          {/* [FIX] Check 'canModify' before showing Write operations */}
          {!isFolder && canModify && (
            <>
